...
//add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/site.css">', 0);
add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/direction-reveal.css">', 0);
?>
...
<div class="acontainer okalbum">
    <div class="direction-reveal direction-reveal--3-grid-flexbox direction-reveal--ok-swing">
        <?php for ($i=0; $i<count($list); $i++) {
          $okcontent = strip_tags($list[$i]['wr_content'],'<br>');
         ?>

          <a href="<?php echo $list[$i]['href'] ?>" class="direction-reveal__card">
            <?php if ($is_checkbox) { ?>
            ...
            <?php
              ...
              if($thumb['src']) {
                $img_content = '<img src="'.$thumb['src'].'" class="img-fluid" alt="'.$thumb['alt'].'" >';
              } else {
            ...
            <div class="direction-reveal__overlay direction-reveal__anim--in">
              <h3 class="direction-reveal__title"><?php echo $list[$i]['subject'] ?></h3>
              <p class="direction-reveal__text"><?php echo conv_subject($okcontent, '180', '...');?><br>
              <?php echo $list[$i]['wr_name'] ?></p>
            </div>
          </a>
        <?php } ?>
        <?php if (count($list) == 0) { echo "<a href='#'>게시물이 없습니다.</a>"; } ?>
    </div>
  </div>
...
<?php } ?>
<script src="<?php echo $board_skin_url;?>/album.js"></script>
<!-- } 게시판 목록 끝 -->
